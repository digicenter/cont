(function (win, doc) {
    ...
    function calcPagamento() {
        $pagamentoValue.value = verificarMax($pagamentoValue.value, 40000);
        calcTotal();
    }

    function calcSocios() {
        $sociosValue.value = verificarMax($sociosValue.value, 20);
        calcTotal();
    }

    function calcFuncionarios() {
        $funcionariosValue.value = verificarMax($funcionariosValue.value, 20);
        calcTotal();
    }
    ...
    function calcTotal() {
        ...
        if ($radioBoxOne.checked) {
            mensalidadeSimples = 199;
        } else if ($radioBoxTwo.checked) {
            mensalidadeSimples = 350;
        }
